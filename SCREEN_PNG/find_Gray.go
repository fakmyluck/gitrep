...
func deletedisp(disp []todisplay) []todisplay {
	...
	fmt.Scan(&key_input)
	vvedenniyind := int(key_input[0] - 48) //sdelat' proverku na "nil?>>?>???"
...
func inputdate() todisplay {
	...
	fmt.Scan(&scankey)
	twoStrings += (" " + scankey)
	disp.start = tim.ToTime(twoStrings)
	...
	fmt.Scan(&scankey)
	twoStrings += (" " + scankey)
	disp.end = tim.ToTime(twoStrings)
...
func strtoui32(s string) uint32 {
	var sum uint32
	i := len(s) - 1
	var mult uint32 = 1
