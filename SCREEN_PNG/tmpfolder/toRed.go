...
func RtoR(img *image.RGBA) image.Image {
...
func openfile(s string) {
	...
	outputFile, err := os.Create("Red/" + s + ".png")
	if err != nil {
		fmt.Println(err)
	}
	switch numPrime.(type) {
	case *image.RGBA:
		e := png.Encode(outputFile, RtoR(numPrime.(*image.RGBA)))
		if e != nil {
	...
	case *image.NRGBA:
		e := png.Encode(outputFile, NRtoR(numPrime.(*image.NRGBA)))
		if e != nil {
			fmt.Println("FAILED to create "+s+"  ", err)
			return
		}
